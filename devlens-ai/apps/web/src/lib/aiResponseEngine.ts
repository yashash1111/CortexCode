...
export function generateAIResponse(prompt: string, mode: string = 'chat', history: any[] = []): string {
  ...
  if (lower.includes('how are you')) {
    return `I'm doing great and ready to help! 🚀 What can I assist you with today?`;
  }
  ...
  // 2. GENERAL QUESTIONS (e.g. 5 steps to improve in life)
...
**Key Rule:** Every recursive function must have a base case to prevent a \`StackOverflowError\`.`;
  ...
  // 4. COMPARISONS (e.g. MongoDB vs PostgreSQL)
...
**Recommendation:** Use **PostgreSQL** for apps needing strict relational structures or financial accuracy. Use **MongoDB** for unstructured data or rapidly evolving document schemas.`;
  ...
  // 5. CODE REQUESTS (e.g. Java Reverse String)
...
- **Space Complexity:** $O(n)$ space for character buffer.`;
  ...
  // 6. DEBUGGING (e.g. NullPointerException)
  if (lower.includes('nullpointer')) {
    return `### Debugging \`NullPointerException\`

A \`NullPointerException\` occurs when attempting to call a method or access an attribute on a \`null\` object reference.
...
// ✅ Fixed with Defensive Null Check
if (str != null) {
    int len = str.length();
}
\`\`\``;
  ...
  // 7. ADAPTIVE DYNAMIC FALLBACK FOR ANY OTHER PROMPT
...
1. **Core Concept:** Ensure modular architecture, explicit type declarations, and proper error handling.
2. **Best Practice:** Keep state management clean and optimize performance for edge cases.

Let me know if you would like a complete code example or step-by-step walkthrough!`;
  }

  return `Here is the response regarding **"${p}"**:

- **Key Goal:** Focus on clean structure, performance optimization, and robust error handling.
- **Next Steps:** Let me know if you'd like a code implementation, tutorial, or specific architecture advice!`;
}
